...
func TestPingIPv4(t *testing.T) {
	...
	assert.NoError(t, err, "Failed to test connectivity between hosts")
}
...
func TestPingDirect(t *testing.T) {
	...
	assert.NoError(t, err, "Failed to show /etc/hosts file")
...
func TestPingMultiHop(t *testing.T) {
	...
	assert.NoError(t, err, "Failed to add router")

	err = g.TestConnectivity(h1, h2)
	assert.NoError(t, err, "Failed to test connectivity between hosts")

	err = h1.Traceroute(h2)
	assert.NoError(t, err, "Failed to traceroute from h1 to h2")
}
